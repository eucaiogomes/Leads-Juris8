  ...
  try {
    ...
    if (req.method === 'GET') {
      try {
        ...
        const payload = { error: err.message || 'Erro ao buscar leads.' };
        if (err.details) payload.details = err.details;
        return sendJson(res, err.status || 502, payload);
      }
    }

    return sendJson(res, 405, { error: 'Method not allowed' });
  } catch (err) {
    console.error('Unexpected error in /api/leads handler:', err);
    return sendJson(res, 500, { error: 'Erro interno no servidor (handler).' });
  }
}
